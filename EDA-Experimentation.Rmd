...
```{r}
# Isolate column with each rivers lat/long points in list
...
ggplot(riversLatLong, aes(x = longitudes, y = latitudes)) + 
  geom_point(size = 0.1)
...
for (x in 1:nrow(raw_rivers)) {
  ...
  for (y in 1:nrow(coordinates)) {
    tempDF <- data.frame(
      "ARCID" = as.integer(raw_rivers[[x, "ARCID"]]), 
  ...
  headRiversExtractRivers <- rbind(headRiversExtractRivers, tempDF)
...
# Why did this work. It should not have worked
...
```{r}
ggplot(headRiversExtractRivers, aes(x = decimalLatitude, y = decimalLongitude)) +
  geom_point()
```

```{r}
write.csv(headRiversExtractRivers, "processed_data/tidied_rivers_extract_example.csv", sep = " ")
```




## Using Haversine to find the nearest river:

```{r}
# Sample data frame with river points
river_data <- data.frame(
  latitude = c(42.361145, 42.360309, 42.359943, 42.359255, 42.358877),
  longitude = c(-71.057083, -71.056835, -71.056510, -71.056396, -71.056089)
)

# Your current latitude and longitude
current_latitude <- 42.360162
current_longitude <- -71.057243
...
distances <- sapply(1:nrow(river_data), function(i) {
  haversine(current_latitude, current_longitude, river_data$latitude[i], river_data$longitude[i])
})
...
nearest_point <- river_data[nearest_point_index, ]

# Print the nearest point and distance
cat("Nearest Point:\n")
cat("Latitude:", nearest_point$latitude, "\n")
cat("Longitude:", nearest_point$longitude, "\n")
cat("Distance (in km):", distances[nearest_point_index], "\n")
